...
from fincopilot.types import ExtractionConfidence, RowMapping, StatementKind
...
US_ALIASES: dict[C, tuple[str, ...]] = {
    ...
    C.COGS: ("cost of sales", "cost of revenues", "cost of revenue", "cost of products sold"),
    C.GROSS_PROFIT: ("gross margin",),
    C.OPERATING_INCOME: ("income from operations", "operating income loss"),
    C.D_AND_A: ("depreciation depletion and amortization",),
    C.NET_INCOME: ("net earnings", "net income loss"),
...
def _is_section_header(table, row) -> bool:
    """A row with a label and no value in any period column."""
    return not any(k[0] == row.ref.ref_id for k in table.cells)
